# Zombie War — Gameplay Design Document (B1, historical)

> **FULLY SUPERSEDED — 2026-08-09.** Canonical gameplay direction now lives in
> [`../../GAME_DESIGN.md`](../../GAME_DESIGN.md), with world architecture in
> [`../../WORLD_STREAMING_TECHNICAL_DESIGN.md`](../../WORLD_STREAMING_TECHNICAL_DESIGN.md).
> Everything below is retained only as historical implementation/asset evidence. It has no design
> authority and must not be used to restore the five authored-map direction or open execution scope.

> **Design history, not current implementation status.** Architecture intent remains useful, but
> counts, phase labels and setup statements may be obsolete. Use `CURRENT_STATE.md` for current facts.
...
- **Máu / hit feedback:** `Health.cs` đơn giản (currentHP, `TakeDamage()`, sự kiện `OnDamaged`/`OnDeath` qua `IEventBus` của BillGameCore) + flash trắng material 1-2 frame + hit-stop ngắn khi trúng đòn nặng + UI health bar world-space hoặc HUD.
- **Current implementation:** weapon handling uses Unity Animation Rigging with a persistent
  ...
  only as decision history; see `../Technical/PlayerRigSocketIncident.md` for the accepted architecture.
...
**Quyết định (bạn chốt trực tiếp):** **không dùng TopDownEngine** (tránh kéo theo cả framework character/ability riêng của nó) — **tự viết bằng Animation Rigging package** (`com.unity.animation.rigging`, đã có sẵn trong manifest từ B2). Code đã viết ở `Weapon.cs`/`WeaponGripPoints.cs`, phần Rig dựng trong Editor (xem `../Technical/EDITOR_SETUP_CHECKLIST.md`):
