...
> **LIVE — ràng buộc kiến trúc, không phải task.** Đợt wiring mô tả ở đây đã thực thi xong, nhưng
> **các "Hard requirements" bên dưới vẫn là luật kiến trúc đang có hiệu lực** — đặc biệt mục 1
> (mọi thứ phải đi qua BillGameCore, không raw `Instantiate`/`FindObjectOfType`/`new AudioSource`)
> và mục 2–4 (zombie theo thừa kế, player spawn-based + map additive, IK tay/chân).
>
> Cách dùng: đọc file này để biết **vì sao** kiến trúc như hiện tại;
> đọc `FRAMEWORK.md` để biết **API thực tế**; đọc `CURRENT_STATE.md` để biết **đã làm tới đâu**.
