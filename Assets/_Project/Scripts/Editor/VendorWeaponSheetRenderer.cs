...
{
    // Renders every weapon prefab across the ThirdParty VENDOR packs into a single labelled
    // contact-sheet PNG so we can eyeball models when mapping WeaponData.
    //
    // Renamed in M7.0 (was `WeaponCatalog`). It never was a catalog: it scans four vendor folders
    // and writes a PNG. The name `WeaponCatalog` now belongs to the authoritative roster asset
    // (`Assets/_Project/Scripts/Runtime/Systems/WeaponCatalog.cs`), which is the single source of
    // truth for weaponId / family / variant / tier / unlock.
    public static class VendorWeaponSheetRenderer
    {
        ...
        [MenuItem("Tools/ZombieWar/Vendor Weapon Sheet")]
